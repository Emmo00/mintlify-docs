...
description: "This documentation provides a comprehensive technical and financial deep dive into the Tokenized Debt product within the BizFi and BizMarket ecosystem, detailing its structure, lifecycle, financial mechanics, liquidity model, and protocol guarantees while abstracting internal operational procedures."
---

Tokenized Debt is the foundational financial instrument within the BizFi protocol. This document provides an external-facing overview of its structure, lifecycle, financial logic, liquidity model, and protocol guarantees, with all internal operational details removed or abstracted.
...
Exit Fee: 10% of remaining value at exit.
...
The BizSwap Vault manages protocol liquidity and settlement. It receives returned BizShares, pays exit USDC, and accounts for fee splits. Vault state management is handled by protocol smart contracts. [INTERNAL ONLY: Detailed vault state transitions and reconciliation steps.]
...
After covering payments, Bitsave pursues recovery offchain. Recovered funds replenish the treasury. Investors are insulated from enforcement complexity. [INTERNAL ONLY: Details of legal, restructuring, or repayment processes.]
